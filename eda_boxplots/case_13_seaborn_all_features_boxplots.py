"""EDA case 13: seaborn boxplots per ticker using extended technical features and 1-day target rolling stats."""
...
def _prepare_feature_frame(subset: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    if subset.empty:
        return pd.DataFrame(), []
    ...
    if feature_frame.empty:
        return feature_frame, available_technical
...
def run_case(
    *,
    ...
    feature_map: dict[str, Mapping[str, Sequence[str]]] = {}
    ...
    for ticker in selected_tickers:
        ...
        if subset.empty:
            ...
            continue
        try:
            feature_frame, used_technical = _prepare_feature_frame(subset)
    ...
    write_case_metadata(
        ...
        extras={
            ...
            "plot_style": "seaborn_horizontal_boxplot_extended_features",
        },
    )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the seaborn per-ticker extended-feature boxplot workflow.",
    )
    parser.add_argument(
        "--dataset",
