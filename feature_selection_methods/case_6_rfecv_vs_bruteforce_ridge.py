"""Case study 6: compare RFECV-selected features with exhaustive combinations using RobustScaler + Ridge."""
...
def _pipeline_coefficients(estimator: Pipeline) -> np.ndarray:
    model = estimator.named_steps.get("model")
    if model is None or not hasattr(model, "coef_"):
        raise AttributeError("Underlying Ridge model does not expose coef_.")
...
def run_case(
    *,
    ...
    """Compare RFECV-selected features with exhaustive combinations using RobustScaler + Ridge."""
    ...
    print(f"Running RFECV (RobustScaler + Ridge) over {len(feature_list)} features with {n_splits} splits...")
    ...
    selector = RFECV(
        ...
        n_jobs=None,
        importance_getter=_pipeline_coefficients,
    )
    selector.fit(X, y)
    ...
    rfecv_metrics["selection_method"] = "RFECV_Robust_Ridge"
    ...
    rfecv_csv = save_dataframe(rfecv_df, case_output_dir, "case_6_rfecv_metrics.csv")
    print(f"RFECV feature metrics saved to: {rfecv_csv}")

    print("Evaluating all feature combinations (brute force) with RobustScaler + Ridge...")
    ...
    for combo in _build_feature_combinations(feature_list):
        ...
        metrics["selection_method"] = "brute_force_robust_ridge"
    ...
    combos_csv = save_dataframe(combos_df, case_output_dir, "case_6_bruteforce_metrics.csv")
    ...
    comparison_df = pd.DataFrame(
        [
            {
                "selection_method": "RFECV_Robust_Ridge",
            ...
            {
                "selection_method": "Brute-force best (Robust Ridge, MAE)",
    ...
    comparison_path = save_dataframe(comparison_df, case_output_dir, "case_6_method_comparison.csv")
    ...
    ax.set_title("RFECV vs brute-force (RobustScaler + Ridge) metrics")
    ax.legend()
    fig.tight_layout()
    comparison_plot_path = case_output_dir / "case_6_method_comparison.png"
