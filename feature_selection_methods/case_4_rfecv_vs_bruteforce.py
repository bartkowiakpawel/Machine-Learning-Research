"""Case study 4: compare RFECV-selected features with exhaustive combinations."""
...
def run_case(
    *,
    ...
    """Compare RFECV-selected features with exhaustive combinations for GradientBoosting."""
    ...
    print(f"Running RFECV over {len(feature_list)} features with {n_splits} splits...")
    ...
    rfecv_metrics["selection_method"] = "RFECV"
    ...
    rfecv_csv = save_dataframe(rfecv_df, case_output_dir, "case_4_rfecv_metrics.csv")
    print(f"RFECV feature metrics saved to: {rfecv_csv}")

    print("Evaluating all feature combinations (brute force)...")
    ...
    for combo in _build_feature_combinations(feature_list):
        ...
        metrics["selection_method"] = "brute_force"
    ...
    combos_csv = save_dataframe(combos_df, case_output_dir, "case_4_bruteforce_metrics.csv")
    print(f"Brute-force combination metrics saved to: {combos_csv}")

    # Visual comparison between RFECV and brute-force best subset by MAE
    ...
    comparison_df = pd.DataFrame(
        [
            ...
            {
                "selection_method": "Brute-force best (MAE)",
    ...
    comparison_path = save_dataframe(comparison_df, case_output_dir, "case_4_method_comparison.csv")
    ...
    ax.set_title("RFECV vs brute-force feature selection metrics")
    ax.legend()
    fig.tight_layout()
    comparison_plot_path = case_output_dir / "case_4_method_comparison.png"
