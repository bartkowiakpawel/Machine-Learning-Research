"""Case study 6: polynomial feature expansion for TSLA."""
...
CASE_ID = "case_6"
CASE_NAME = "TSLA polynomial feature expansion"
...
def run_case(
    *,
    ...
    """Run polynomial-feature-focused scaling workflow for a single ticker."""
    ...
    if not CASE_FEATURES:
        raise ValueError("CASE_FEATURES configuration is empty for case_6.")
    if CASE_TARGET is None:
        raise ValueError("CASE_TARGET configuration is missing for case_6.")
    ...
    raw_matrix_path = save_dataframe(
        feature_matrix,
        case_output_dir,
        f"{ticker.lower()}_case6_feature_matrix_raw.csv",
    )
    ...
    raw_summary_path = save_dataframe(
        ...
        case_output_dir,
        f"{ticker.lower()}_case6_feature_matrix_raw_summary.csv",
    )
    ...
    raw_results, raw_curve_path, _ = run_learning_workflow(
        ...
        X_raw,
        y_raw,
        title_suffix=f"{ticker} (case6 raw)",
    ...
    raw_results_path = save_dataframe(
        raw_results,
        case_output_dir,
        f"{ticker.lower()}_case6_learning_curve_raw.csv",
    )
    ...
    if metadata_for_validation is not None:
        _run_last_n_validation(
            ...
            dataset_label=f"{ticker} case6 raw",
            output_dir=last_n_output_dir,
            plot_prefix=f"{ticker.lower()}_case6_raw",
            model_store_dir=model_store_dir,
        )

    print("--- Applying polynomial feature expansion ---")
    polynomial_transformer = PolynomialFeatures(
        degree=POLYNOMIAL_DEGREE,
        include_bias=False,
    )
    X_polynomial_array = polynomial_transformer.fit_transform(X_raw)
    feature_names = polynomial_transformer.get_feature_names_out(CASE_FEATURES)
    ...
    polynomial_matrix_path = save_dataframe(
        polynomial_matrix,
        case_output_dir,
        f"{ticker.lower()}_case6_feature_matrix_polynomial.csv",
    )
    ...
    polynomial_summary_path = save_dataframe(
        ...
        case_output_dir,
        f"{ticker.lower()}_case6_feature_matrix_polynomial_summary.csv",
    )
    print(f"Polynomial summary saved to: {polynomial_summary_path}")

    print("--- Learning curves on polynomial features ---")
    polynomial_results, polynomial_curve_path, _ = run_learning_workflow(
        MODEL_DICT,
        X_polynomial,
        y_raw,
        title_suffix=f"{ticker} (case6 polynomial)",
        output_dir=case_output_dir,
        show_plots=show_plots,
    )
    polynomial_results_path = save_dataframe(
        polynomial_results,
        case_output_dir,
        f"{ticker.lower()}_case6_learning_curve_polynomial.csv",
    )
    ...
    if metadata_for_validation is not None:
        poly_transformer_for_validation = PolynomialFeatures(
            degree=POLYNOMIAL_DEGREE,
            include_bias=False,
        )
        _run_last_n_validation(
            ...
            y_raw,
            metadata=metadata_for_validation,
            dataset_label=f"{ticker} case6 polynomial",
            output_dir=last_n_output_dir,
            plot_prefix=f"{ticker.lower()}_case6_polynomial",
            model_store_dir=model_store_dir,
            preprocessor=poly_transformer_for_validation,
        )

    polynomial_distribution_path = plot_features_distribution_grid(
        X_polynomial.loc[:, CASE_FEATURES],
        X_raw,
        title=f"{ticker} features: case6 Polynomial vs raw",
        output_dir=case_output_dir,
    )
    ...
    comparison_results = pd.concat(
        [
            raw_results.assign(dataset="case6_raw"),
            polynomial_results.assign(dataset="case6_polynomial"),
        ],
        ignore_index=True,
    )
    comparison_path = save_dataframe(
        comparison_results,
        case_output_dir,
        f"{ticker.lower()}_case6_learning_curve_comparison.csv",
    )
