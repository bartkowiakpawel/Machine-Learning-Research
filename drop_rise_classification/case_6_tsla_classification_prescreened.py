"""Case 6: TSLA drop/rise classification with feature screening prior to RFECV."""
...
def run_case(
    *,
    ...
    try:
        ...
        X_full_selected = classification_df.loc[:, selected_features].to_numpy(dtype=float)
        y_full = label_encoder.transform(classification_df[TARGET_COLUMN])

        rise_label = "rise"
        if rise_label in label_encoder.classes_:
            rise_index = int(np.where(label_encoder.classes_ == rise_label)[0][0])
        else:  # pragma: no cover - defensive fallback
            rise_index = 0

        oof_splits = min(cv_splits, len(classification_df) - 1)
        if oof_splits < 2:
            oof_splits = 2 if len(classification_df) > 2 else 1

        gbm_oof = np.full(len(classification_df), np.nan, dtype=float)
        logreg_oof = np.full(len(classification_df), np.nan, dtype=float)

        if oof_splits > 1:
            tscv_oof = TimeSeriesSplit(n_splits=oof_splits)
            for train_idx, val_idx in tscv_oof.split(X_full_selected):
                unique_classes = np.unique(y_full[train_idx])
                if unique_classes.shape[0] < 2:
                    continue

                gbm_clone = clone(estimators["GradientBoosting"])
                gbm_clone.fit(X_full_selected[train_idx], y_full[train_idx])
                gbm_classes = getattr(gbm_clone, "classes_", None)
                if gbm_classes is not None and rise_label in gbm_classes:
                    gbm_class_map = {cls: idx for idx, cls in enumerate(gbm_classes)}
                    gbm_oof[val_idx] = gbm_clone.predict_proba(X_full_selected[val_idx])[
                        :, gbm_class_map[rise_label]
                    ]

                logreg_clone = clone(estimators["LogisticRegression"])
                logreg_clone.fit(X_full_selected[train_idx], y_full[train_idx])
                logreg_classes = getattr(logreg_clone, "classes_", None)
                if logreg_classes is not None and rise_label in logreg_classes:
                    logreg_class_map = {cls: idx for idx, cls in enumerate(logreg_classes)}
                    logreg_oof[val_idx] = logreg_clone.predict_proba(X_full_selected[val_idx])[
                        :, logreg_class_map[rise_label]
                    ]

        gbm_full_pipeline = clone(estimators["GradientBoosting"])
        gbm_full_pipeline.fit(X_full_selected, y_full)
        gbm_full_classes = getattr(gbm_full_pipeline, "classes_", None)
        if gbm_full_classes is not None and rise_label in gbm_full_classes:
            gbm_class_map_full = {cls: idx for idx, cls in enumerate(gbm_full_classes)}
            gbm_full_proba = gbm_full_pipeline.predict_proba(X_full_selected)[:, gbm_class_map_full[rise_label]]
        else:
            gbm_full_proba = np.full(len(classification_df), np.nan, dtype=float)

        logreg_full_pipeline = clone(estimators["LogisticRegression"])
        logreg_full_pipeline.fit(X_full_selected, y_full)
        logreg_full_classes = getattr(logreg_full_pipeline, "classes_", None)
        if logreg_full_classes is not None and rise_label in logreg_full_classes:
            logreg_class_map_full = {cls: idx for idx, cls in enumerate(logreg_full_classes)}
            logreg_full_proba = logreg_full_pipeline.predict_proba(X_full_selected)[
                :, logreg_class_map_full[rise_label]
            ]
        else:
            logreg_full_proba = np.full(len(classification_df), np.nan, dtype=float)
        logreg_full_log_odds = _compute_log_odds(logreg_full_proba)

        logreg_oof_log_odds = np.full(len(classification_df), np.nan, dtype=float)
        valid_oof_mask = ~np.isnan(logreg_oof)
        if np.any(valid_oof_mask):
            logreg_oof_log_odds[valid_oof_mask] = _compute_log_odds(logreg_oof[valid_oof_mask])
        ...
        feature_enriched_frame["rise_proba_gbm"] = gbm_oof
        feature_enriched_frame["log_odds_logreg"] = logreg_oof_log_odds
        feature_enriched_frame["rise_proba_gbm_full"] = gbm_full_proba
        feature_enriched_frame["log_odds_logreg_full"] = logreg_full_log_odds
        ...
        metadata_extras: dict[str, object] = {
            ...
            "oof_generated_features": ["rise_proba_gbm", "log_odds_logreg"],
            "full_fit_generated_features": ["rise_proba_gbm_full", "log_odds_logreg_full"],
            "oof_time_series_splits": int(oof_splits) if oof_splits > 1 else None,
            "confusion_matrices": {
