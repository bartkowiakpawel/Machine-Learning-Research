"""Case 7: TSLA regression with feature screening using Case 6 classification dataset."""
...
DEFAULT_DATASET_PATH = (
    Path("drop_rise_classification") / "outputs" / "case_6" / "tsla_case_6_feature_enriched_dataset.csv"
)
...
def _resolve_dataset_path(dataset_arg: str | None) -> Path:
    ...
    raise FileNotFoundError(
        "Unable to locate the regression dataset. Provide --dataset pointing to the Case 6 feature enriched CSV."
    )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="dataset",
        default=None,
        help="Path to the regression dataset CSV (default: Case 6 feature enriched dataset).",
    )
    parser.add_argument(
        "--ticker",
        dest="ticker",
