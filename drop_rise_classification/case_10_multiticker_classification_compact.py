"""Case 10: Multi-ticker classification with compact feature expansion baseline."""
...
from core.feature_expansion import TARGET_FEATURE_COLUMNS
from core.feature_expansion_compact import compact_expand_base_features
...
def _prepare_multiticker_classification_dataset(
    ...
    *,
    ...
    top_mutual_info: int | None,
    mutual_info_random_state: int,
) -> tuple[pd.DataFrame, list[str], list[str], list[str], dict[str, list[str]]]:
    ...
    expansion_result = compact_expand_base_features(
        working,
        ...
        top_n_mutual_info=None,
        mutual_info_target=None,
        mutual_info_random_state=mutual_info_random_state,
        exclude_from_features=("ticker", "Date"),
        remove_forward_targets=False,
    )

    enriched = expansion_result.frame.replace([np.inf, -np.inf], pd.NA)
    ...
    dropped_post_mi: list[str] = []
    if top_mutual_info is not None and top_mutual_info > 0 and feature_columns:
        top_limit = min(top_mutual_info, len(feature_columns))
        if top_limit < len(feature_columns):
            feature_matrix = classification_frame.loc[:, feature_columns].to_numpy(dtype=float)
            target_codes = classification_frame[TARGET_COLUMN].cat.codes.to_numpy()
            mi_scores = mutual_info_classif(
                feature_matrix,
                target_codes,
            ...
            selected = {feature_columns[idx] for idx in order[:top_limit]}
            dropped_post_mi = [feature for feature in feature_columns if feature not in selected]
            feature_columns = [feature for feature in feature_columns if feature in selected]
            classification_frame = classification_frame.drop(columns=dropped_post_mi, errors="ignore")

    if not feature_columns:
        raise ValueError("No features remaining after compact expansion and mutual information filtering.")
    ...
    aux_feature_groups = {
        "base": base_features,
        "target": target_feature_columns,
        "derived": derived_feature_columns,
        "dropped_pre_screen": expansion_result.dropped_pre_screen,
        "dropped_variance": expansion_result.dropped_variance,
        "dropped_mutual_info_initial": expansion_result.dropped_mutual_info,
        "dropped_mutual_info_final": dropped_post_mi,
        "compact_config": dict(expansion_result.config),
    }

    return classification_frame, feature_columns, base_features, target_feature_columns, derived_feature_columns, aux_feature_groups
...
def run_case(
    *,
    ...
    top_mutual_info: int | None = None,
    mutual_info_random_state: int = 0,
    enable_distribution_diagnostics: bool = False,
) -> Path:
    """Execute multi-ticker classification with compact feature expansion prior to RFECV."""
    ...
    try:
        ...
        variance_threshold_effective = None if variance_threshold is not None and variance_threshold <= 0 else variance_threshold
        top_mutual_info_effective = None if top_mutual_info is None or top_mutual_info <= 0 else int(top_mutual_info)
        ...
        (
            ...
            variance_threshold=variance_threshold_effective,
            top_mutual_info=top_mutual_info_effective,
            mutual_info_random_state=mutual_info_random_state,
        )
        ...
        metadata_extras: dict[str, object] = {
            ...
            "compact_variance_threshold": variance_threshold_effective,
            "compact_top_mutual_info": top_mutual_info_effective,
            "compact_mutual_info_random_state": mutual_info_random_state,
            ...
            "compact_feature_tracking": aux_feature_groups,
            "confusion_matrices": {
        ...
        write_case_metadata(
            ...
            features={
                ...
                "base_features": list(base_features_used),
                "target_features": list(target_features_used),
                "derived_features": list(derived_features_used),
            },
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-5,
        help="Variance threshold applied during compact feature expansion (set <=0 to disable).",
    )
    parser.add_argument(
        "--top-mi-features",
        dest="top_mutual_info",
        type=int,
        default=None,
        help="Optional cap on feature count after mutual information ranking (<=0 to disable).",
    )
    parser.add_argument(
        "--mi-random-state",
        dest="mutual_info_random_state",
        type=int,
        default=0,
        help="Random seed used during mutual information ranking.",
    )
    parser.add_argument(
...
def main() -> None:  # pragma: no cover - CLI convenience
    ...
    run_case(
        ...
        top_mutual_info=args.top_mutual_info,
        mutual_info_random_state=args.mutual_info_random_state,
